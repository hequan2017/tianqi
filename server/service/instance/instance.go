...
func (instanceService *InstanceService) CreateInstance(ctx context.Context, inst *instanceModel.Instance) (err error) {
	...
	// 扩展参数：宿主机端口映射与启动命令（仅内部调用使用）
	if inst.HostPort != nil && *inst.HostPort > 0 {
		containerConfig.HostPort = *inst.HostPort
	}
	if inst.StartupCmd != nil && strings.TrimSpace(*inst.StartupCmd) != "" {
		containerConfig.StartupCmd = []string{"sh", "-c", *inst.StartupCmd}
	}
