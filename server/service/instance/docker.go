...
func (d *DockerService) CreateContainer(ctx context.Context, node *computenode.ComputeNode, config *ContainerConfig) (containerID string, err error) {
	...
	if len(config.StartupCmd) > 0 {
		containerConfig.Cmd = config.StartupCmd
	}
	...
	if err != nil && strings.Contains(strings.ToLower(err.Error()), "no such image") {
		global.GVA_LOG.Warn("镜像不存在，开始拉取镜像", zap.String("image", config.Image), zap.Error(err))
		if pullErr := d.pullImage(ctx, cli, config.Image); pullErr != nil {
			return "", fmt.Errorf("拉取镜像失败: %v", pullErr)
		}
		resp, err = cli.ContainerCreate(ctx, containerConfig, hostConfig, nil, nil, config.Name)
	}
...
func (d *DockerService) pullImage(ctx context.Context, cli *client.Client, imageRef string) error {
