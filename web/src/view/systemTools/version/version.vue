  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="版本描述" min-width="200">
    ...
    <el-drawer v-model="importDialogVisible" title="导入版本" direction="rtl" size="80%" :before-close="closeImportDialog" :show-close="false">
      ...
      <el-form label-width="100px">
        <el-form-item label="上传文件">
          ...
          >
            ...
            <div class="el-upload__text">
              将JSON文件拖到此处，或<em>点击上传</em>（支持多选）
            </div>
            <template #tip>
              <div class="el-upload__tip">
                支持上传多个JSON文件，每个文件将作为单独的版本导入
              </div>
            </template>
          </el-upload>
        </el-form-item>
        <el-form-item label="待导入列表" v-if="importFileList.length > 0">
          <div class="w-full border border-gray-300 rounded overflow-hidden">
            <el-table :data="importFileList" style="width: 100%">
              <el-table-column prop="name" label="文件名" min-width="200" />
              <el-table-column label="版本信息" min-width="180">
                <template #default="scope">
                  <span v-if="scope.row.versionInfo">
                    {{ scope.row.versionInfo.name }} ({{ scope.row.versionInfo.code }})
                  </span>
                  <span v-else class="text-gray-400">解析中...</span>
                </template>
              </el-table-column>
              <el-table-column label="状态" width="100">
                <template #default="scope">
                  <el-tag v-if="scope.row.status === 'pending'" type="info" size="small">待导入</el-tag>
                  <el-tag v-else-if="scope.row.status === 'importing'" type="warning" size="small">导入中</el-tag>
              ...
              <el-table-column label="操作" width="140" fixed="right">
                <template #default="scope">
                  <el-button type="primary" link size="small" @click="previewImportFile(scope.$index)">预览</el-button>
                  <el-button type="danger" link size="small" @click="removeImportFile(scope.$index)">移除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
        ...
        <el-form-item label="版本JSON">
          <el-input v-model="importJsonContent" type="textarea" :rows="15" placeholder="请粘贴版本JSON（也可直接上传文件）"
...
const uploadFileList = ref([]) // 多文件上传列表
const currentPreviewIndex = ref(0) // 当前预览的文件索引
const importFileList = ref([]) // 待导入文件列表
  ...
  reader.onload = (e) => {
    try {
      ...
      const fileInfo = {
        ...
        data: parsedData,
        versionInfo: {
          name: parsedData.version?.name || parsedData.versionInfo?.name || '未知版本',
          code: parsedData.version?.code || parsedData.versionInfo?.code || '未知版本号'
        },
        status: 'pending' // pending, importing, success, error
      }

      // 检查是否已存在同名文件
      const existingIndex = importFileList.value.findIndex(f => f.name === file.name)
      if (existingIndex > -1) {
        importFileList.value[existingIndex] = fileInfo
      } else {
        importFileList.value.push(fileInfo)
      }

      // 如果是第一个文件，自动预览
      if (importFileList.value.length === 1) {
        previewImportFile(0)
      }

      ElMessage.success(`文件 ${file.name} 解析成功`)
    } catch (error) {
      ElMessage.error(`文件 ${file.name} JSON格式错误`)
    }
  }
...
// 预览指定文件
const previewImportFile = (index) => {
  if (index < 0 || index >= importFileList.value.length) return

  currentPreviewIndex.value = index
  const file = importFileList.value[index]
  importJsonContent.value = file.content
...
const handleFileRemove = (file, _fileList) => {
  // 从importFileList中移除对应的文件
...
const handleImport = async () => {
  ...
  if (importFileList.value.length > 0) {
    ...
    for (let i = 0; i < importFileList.value.length; i++) {
      const file = importFileList.value[i]
      file.status = 'importing'

      try {
        const res = await importVersion(file.data)
        if (res.code === 0) {
          file.status = 'success'
          successCount++
        } else {
          file.status = 'error'
          errorCount++
          console.error(`文件 ${file.name} 导入失败:`, res.msg)
        }
      } catch (error) {
        file.status = 'error'
        errorCount++
    ...
    if (successCount > 0) {
      ElMessage.success(`成功导入 ${successCount} 个版本${errorCount > 0 ? `，失败 ${errorCount} 个` : ''}`)
      closeImportDialog()
      getTableData() // 刷新表格数据
    } else {
  ...
  // 否则使用文本框内容导入
  if (!importJsonContent.value.trim()) {
    ElMessage.warning('请输入版本JSON或上传文件')
    return
  }
